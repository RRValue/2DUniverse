...
Point* const ControllerCutSpline::getPointAtPos(const Vec2d& pos, size_t* const idx) const
{
    if(m_Points.empty())
        return nullptr;
...
void ControllerCutSpline::cut()
{
    ...
    IntersectionVector intersections = m_Line->intersect(m_Spline);
